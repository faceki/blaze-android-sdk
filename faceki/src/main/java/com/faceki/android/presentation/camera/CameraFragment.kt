...
internal class CameraFragment : BaseFragment<FragmentCameraBinding>(FragmentCameraBinding::inflate),
    ...
    private val displayListener = object : DisplayManager.DisplayListener {
        ...
        override fun onDisplayChanged(displayId: Int) = view?.let { view ->
            if (displayId == this@CameraFragment.displayId) {
                Log.d(AppConfig.TAG, "Rotation changed: " + view.display.rotation)
    ...
    private fun bindCameraUseCases() {
        ...
        Log.d(AppConfig.TAG, "Screen metrics: " + metrics.width() + " x " + metrics.height())

        val screenAspectRatio = aspectRatio(metrics.width(), metrics.height())
        Log.d(AppConfig.TAG, "Preview aspect ratio: $screenAspectRatio")
        ...
        try {
            ...
            Log.e(AppConfig.TAG, "${exc.localizedMessage} ,Use case binding failed")
        }
    }
            ...
            run {
                when (cameraState.type) {
                    CameraState.Type.PENDING_OPEN -> {
                        ...
                        Log.i(AppConfig.TAG, "CameraState: Pending Open")
                        showToast("Close other camera apps")
                    }

                    CameraState.Type.OPENING -> {
                        // Show the Camera UI
                        Log.i(AppConfig.TAG, "CameraState: Opening")
                    }

                    CameraState.Type.OPEN -> {
                        // Setup Camera resources and begin processing
                        Log.i(AppConfig.TAG, "CameraState: Open")
                    }

                    CameraState.Type.CLOSING -> {
                        // Close camera UI
                        Log.i(AppConfig.TAG, "CameraState: Closing")
                    }

                    CameraState.Type.CLOSED -> {
                        // Free camera resources
                        Log.i(AppConfig.TAG, "CameraState: Closed")
                    }
                }
            }
            ...
            cameraState.error?.let { error ->
                when (error.code) {
                    ...
                    CameraState.ERROR_MAX_CAMERAS_IN_USE -> {
                        ...
                        Log.i(AppConfig.TAG, "Max cameras in use")
                        showToast("Close other camera apps")
                    }

                    CameraState.ERROR_OTHER_RECOVERABLE_ERROR -> {
                        Log.i(AppConfig.TAG, "Other recoverable error")
                    }
                    // Closing errors
                    CameraState.ERROR_CAMERA_DISABLED -> {
                        // Ask the user to enable the device's cameras
                        Log.i(AppConfig.TAG, "Camera disabled")
                        showToast("Enable the device's cameras")
                    }

                    CameraState.ERROR_CAMERA_FATAL_ERROR -> {
                        // Ask the user to reboot the device to restore camera function
                        Log.i(AppConfig.TAG, "Fatal error")
                        showToast("reboot the device to restore camera function")
                    }
                    // Closed errors
                    CameraState.ERROR_DO_NOT_DISTURB_MODE_ENABLED -> {
                        // Ask the user to disable the "Do Not Disturb" mode, then reopen the camera
                        Log.i(AppConfig.TAG, "Do not disturb mode enabled")
...
//        )
    ...
    private fun updateCameraSwitchButton() {
        try {
            ...
            Log.e(AppConfig.TAG, exception.localizedMessage ?: "")
        }
    }
                    ...
                    imageCapture.takePicture(
                        cameraExecutor!!, object : ImageCapture.OnImageCapturedCallback() {
                            override fun onError(exc: ImageCaptureException) {
                                Log.e(
                                    AppConfig.TAG,
                                    "${exc.localizedMessage} , Photo capture failed: " + exc.message
                                )
                                capturedImage.delete()
                                capturedImage = null
                                runOnUiThread {
